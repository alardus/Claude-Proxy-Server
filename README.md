...
**Примечания:**
- `OPENAI_API_KEY` требуется только если вы планируете использовать проксирование для OpenAI API
- `DEBUG_MODE=true` включает детальное логирование запросов и ответов (полезно для отладки)
...
Для включения детального логирования запросов и ответов установите в `.env`:

```env
DEBUG_MODE=true
```

В режиме отладки в логах будут отображаться:
- Детали входящих запросов (пути, параметры, количество сообщений)
- Наличие файлов в запросах
- Добавленные beta-заголовки
- Статусы и заголовки ответов от API
- Первые 10 чанков данных при streaming
- Детальная информация об ошибках

**Важно:** В продакшене рекомендуется отключать DEBUG_MODE для производительности и безопасности.
